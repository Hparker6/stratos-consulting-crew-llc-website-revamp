...
export default function Outcomes() {
  return (
    <section id="outcomes" className="py-20 lg:py-28 bg-bg">
      <div className="max-w-6xl mx-auto px-5 text-center">
        <p className="eyebrow text-secondary mb-4">The outcome</p>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 text-left">
          {outcomes.map((o) => (
            <article key={o.title} className="card p-7 hover:border-white/[0.14] transition-colors">
              <CheckTile />
              <h3 className="font-heading font-bold text-[16px] text-text-base mt-4 mb-2">{o.title}</h3>
              <p className="text-muted text-[14px] leading-relaxed">{o.body}</p>
            </article>
          ))}
        </div>
      </div>
