  ...
  return (
    <section className="relative overflow-hidden" style={{ background: '#0a0f1c' }}>
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(47,143,255,0.12) 0%, transparent 65%)',
          filter: 'blur(40px)',
        }}
      />
      <div
        ...
        style={{
          background: 'radial-gradient(circle, rgba(39,224,160,0.08) 0%, transparent 65%)',
          filter: 'blur(50px)',
        }}
      />
      ...
      <div className="relative max-w-6xl mx-auto px-5 pt-14 pb-12 lg:pt-20 lg:pb-16">
        <p className="eyebrow text-secondary mb-3">{eyebrow}</p>
        <h1 className="font-heading font-extrabold text-[36px] md:text-[50px] leading-[1.08] tracking-[-0.03em] text-text-base max-w-[760px]">
          {title}
        </h1>
        {lede && (
          <p className="mt-5 text-muted font-medium text-[17px] leading-relaxed max-w-[620px]">{lede}</p>
        )}
      </div>
    </section>
  )
